...
impl WifiController {
    pub async fn new() -> Self {
        ...
        match Connection::system().await {
            ...
            Err(err) => {
                warn!("Could not connect to System DBus ({}). Using mock fallback.", err);
                Self { use_mock: true }
            }
        }
    }
    ...
    pub async fn scan_networks(&self) -> Result<Vec<WifiNetwork>, String> {
        if self.use_mock {
            return Ok(vec![
                WifiNetwork { ssid: "Klipper-Secure-5G".to_string(), signal: 94, security: "WPA2/WPA3".to_string() },
                WifiNetwork { ssid: "Workshop_Guest".to_string(), signal: 62, security: "WPA2".to_string() },
                WifiNetwork { ssid: "Neighborhood-Printer".to_string(), signal: 35, security: "Open".to_string() },
            ]);
        }
        ...
        let _reply: zbus::zvariant::OwnedValue = conn.call_method(
            Some("org.freedesktop.NetworkManager"),
            "/org/freedesktop/NetworkManager",
            Some("org.freedesktop.NetworkManager"),
            "GetDevices",
            &(),
        ).await.map_err(|e| e.to_string())?.body().deserialize().map_err(|e| e.to_string())?;
        ...
        Ok(vec![
            WifiNetwork { ssid: "Klipper-DBus-WiFi".to_string(), signal: 88, security: "WPA2".to_string() },
            WifiNetwork { ssid: "NM-Connected-AP".to_string(), signal: 75, security: "WPA2".to_string() },
        ])
    }

    pub async fn connect_to_network(&self, ssid: &str, password: &str) -> Result<(), String> {
        if self.use_mock {
            info!("Mock connecting to SSID: {} with password: {}", ssid, "*".repeat(password.len()));

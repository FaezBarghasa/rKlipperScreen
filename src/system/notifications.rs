...
pub async fn send_system_notification(summary: &str, body: &str) -> Result<(), zbus::Error> {
    ...
    let conn = match Connection::session().await {
        Ok(c) => c,
        Err(err) => {
            warn!("Could not connect to Session DBus for notifications: {}", err);
            return Ok(());
        }
    };
    ...
    let hints: std::collections::HashMap<String, zbus::zvariant::Value<'_>> = std::collections::HashMap::new();
    let expire_timeout = -1i32; // Default expiration

    let _reply: u32 = conn.call_method(
